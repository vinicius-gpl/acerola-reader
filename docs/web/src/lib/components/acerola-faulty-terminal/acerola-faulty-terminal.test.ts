...
describe('AcerolaFaultyTerminal', () => {
	beforeEach(() => {
	...
	it('mounts and appends the WebGL canvas to its container', () => {
		const { container } = render(AcerolaFaultyTerminal, { props: { tint: '#cba6f7' } });

		expect(container.querySelector('canvas')).toBeInTheDocument();
	});

	it('releases the WebGL context on unmount', () => {
		const { unmount, container } = render(AcerolaFaultyTerminal);
		expect(container.querySelector('canvas')).toBeInTheDocument();
	...
	it('accepts a hex tint shorthand without throwing', () => {
		expect(() => render(AcerolaFaultyTerminal, { props: { tint: '#fff' } })).not.toThrow();
	});
});
