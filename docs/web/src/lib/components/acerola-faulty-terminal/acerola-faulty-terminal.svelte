	...
	function colorToRgb(color: string): [number, number, number] {
		if (color.startsWith('rgb')) {
			...
			if (matches && matches.length >= 3) {
				return [Number(matches[0]) / 255, Number(matches[1]) / 255, Number(matches[2]) / 255];
			}
		}
...
<script lang="ts">
	...
	const actualTimeScale = $derived(speed ?? timeScale);
	const actualNoiseAmp = $derived(noiseAmplitude ?? noiseAmp);
	const actualTint = $derived(tintColor ?? tint);
	...
	const tintVec = $derived(colorToRgb(actualTint));
	...
	$effect(() => {
		if (program) {
			...
			program.uniforms.uNoiseAmp.value = actualNoiseAmp;
	...
	$effect(() => {
		...
		const renderer = new Renderer({ dpr: init.dpr, alpha: true, premultipliedAlpha: false });
		...
		program = new Program(gl, {
			...
			uniforms: {
				...
				uGridMul: { value: new Float32Array(init.gridMul) },
				uDigitSize: { value: init.digitSize },
				uScanlineIntensity: { value: init.scanlineIntensity },
				uGlitchAmount: { value: init.glitchAmount },
				uFlickerAmount: { value: init.flickerAmount },
				uNoiseAmp: { value: init.noiseAmp },
				uChromaticAberration: { value: init.chromaticAberration },
				uDither: { value: init.dither },
				uCurvature: { value: init.curvature },
				uTint: { value: new Color(init.tintVec[0], init.tintVec[1], init.tintVec[2]) },
				uMouse: { value: new Float32Array([smoothMouse.x, smoothMouse.y]) },
				uMouseStrength: { value: init.mouseStrength },
				uUseMouse: { value: init.mouseReact ? 1 : 0 },
				uPageLoadProgress: { value: init.pageLoadAnimation ? 0 : 1 },
				uUsePageLoadAnimation: { value: init.pageLoadAnimation ? 1 : 0 },
		...
		const update = (t: number) => {
			...
			if (!pause) {
				const elapsed = (t * 0.001 + timeOffset) * actualTimeScale;
