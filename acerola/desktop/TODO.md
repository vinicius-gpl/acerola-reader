...
- [ ] **[Alta] [To Fix] Recompilação do app quebra o handshake P2P e perde conexões antigas** - Sempre que o app é recompilado/atualizado, o handshake do P2P quebra com peers já conhecidos e as conexões antigas são perdidas (possível rotação/perda da identidade criptográfica do nó, invalidação de chaves/tickets ou ausência de negociação resiliente entre a nova instância compilada e os peers antigos). Necessário investigar a persistência da SecretKey/NodeId e implementar recuperação automática do handshake sem forçar o usuário a refazer o pareamento do zero.
